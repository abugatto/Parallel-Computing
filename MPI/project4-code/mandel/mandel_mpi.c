...
int main (int argc, char** argv)
{
	...
	MPI_ERR_CHECK(MPI_Comm_rank(MPI_COMM_WORLD, &mpi_rank));
	MPI_ERR_CHECK(MPI_Comm_size(MPI_COMM_WORLD, &mpi_size));
	...
	if (mpi_rank != 0)
	{
		...
		MPI_ERR_CHECK(MPI_Send(c, d.nx * d.ny, MPI_INT, 0, mpi_rank, p.comm));
	}
	/****************************************************************************/
	// Write the image
	if (mpi_rank == 0)
	{
		...
		for(int proc = 1; proc < mpi_size; proc++)
		{
			...
			MPI_ERR_CHECK(MPI_Recv(c, d.nx * d.ny, MPI_INT, proc, proc, p.comm, MPI_STATUS_IGNORE));
	...
	MPI_ERR_CHECK(MPI_Comm_free(&p.comm));
	free(c);
	MPI_Finalize();
	return 0;
}
